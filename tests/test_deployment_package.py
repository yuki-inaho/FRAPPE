...
REQUIRED = [
    ...
    "encoder_int8_64x64.xml",
    "encoder_int8_64x64.bin",
    "decoder.onnx",
    "package_report.json",
]
...
def package(model, tmp_path, extra_args=()):
    ...
    torch.save(
        {"config": vars(make_config()), "model": model.state_dict(), "iteration": 4},
        tmp_path / "base.pth.tar",
    )
...
def test_qdq_package_keeps_the_codec_boundary_out_of_qdq(model, tmp_path):
    ...
    assert all("/companders." not in node.name for node in quantizers + weight_dequantizers)
    fp32_ops = [node.op_type for node in onnx.load(str(output / "encoder_fp32.onnx")).graph.node]
...
def test_static_ir_carries_the_frozen_resolution(model, tmp_path):
    ...
    assert [
        tuple(result.shape)
        for result in ov.Core().compile_model(ir, "CPU")(
...
def test_decoder_ships_in_the_same_package(model, tmp_path):
    ...
    session = onnxruntime.InferenceSession(
        str(output / "decoder.onnx"), providers=["CPUExecutionProvider"]
    )
