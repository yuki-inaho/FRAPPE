...
The library is looked up from the deployment environment's ``lib`` directory
and then from the documented system requirement (``libcharls2``); it fails
loud when a caller explicitly asked for the native backend and soft for a
probe: :func:`available` is False when it is missing and
:func:`unavailable_reason` says why. If the C API someday disagrees with the
PIL path, the byte-identity tests fail before any rate number can silently
shift.
"""
...
def _candidate_paths():
    """The documented homes of libcharls: the env's lib dir, then the system."""
    yield Path(sys.prefix) / "lib" / "libcharls.so.2"
    yield "libcharls.so.2"  # resolved by ldconfig, e.g. /usr/lib/x86_64-linux-gnu
...
def unavailable_reason() -> str | None:
    """Why :func:`available` is False, or None when the library resolves."""
    _load()
    return _state["error"]
