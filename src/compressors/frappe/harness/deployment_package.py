...
def export_graph(module: torch.nn.Module, sample: tuple, path: Path,
                 input_names: list[str], output_names: list[str], shapes, opset: int = 18) -> dict:
...
def build_package(model: JointPrefixFRAPPE, checkpoint_path: Path,
                  dataset_root: Path, output_dir: Path, *,
                  calibration_split: str, calibration_images: int,
                  verify_split: str, verify_images: int, height: int, width: int,
                  prefix: int | None = None, target_device: str = "NPU",
                  ptq_preset: str = "performance", bias_correction: str = "fast",
                  decode_device: str | torch.device = "cpu",
                  checkpoint_iteration: int | None = None) -> dict:
    ...
    with torch.no_grad():
        sample_planes = tuple(plane.clone() for plane in
                              EncoderGraph(model, uint8_io=True)(sample_image))
    ...
    decoder_info = export_graph(DecoderGraph(model, prefix, uint8_io=True).eval(),
                                sample_planes, decoder_path, names, ["reconstruction"],
                                (plane_shapes,))
    ...
    quantization = quantize_onnx_encoder(
        fp32_path, qdq_path, calibration,
    ...
    conditions = measure_deployed_conditions(
        compiled, decoder, verify_folder, verify_indices,
        scale_groups=model.scale_groups, plan=plan, prefix=prefix,
        height=height, width=width, device=torch.device(decode_device))

    artifacts = {path.name: {"sha256": sha256_of(path), "bytes": path.stat().st_size}
                 for path in (fp32_path, qdq_path, Path(ir_info["xml"]), Path(ir_info["bin"]),
                              decoder_path)}
    manifest = {
        "artifacts": artifacts,
        ...
        "quantization_boundary": {
            ...
            "preset": ptq_preset, "target_device": target_device.upper(),
            "bias_correction": bias_correction,
        ...
        "rd_baseline": {
            "split": verify_split, "image_indices": verify_indices,
            "size": [width, height], "bitstream_convention": "PAYLOAD_ONLY",
            "averaging": "aggregate_mse",
            "decoder": "torch DecoderGraph (frozen, fp32)",
            "conditions": conditions,
            "deltas_vs_fp32": {
                "int8": {"d_psnr_db": conditions["int8"]["psnr_db"]
                                  - conditions["fp32"]["psnr_db"],
                         "d_bpp": conditions["int8"]["bpp"] - conditions["fp32"]["bpp"]}},
        },
        "versions": {"torch": torch.__version__, "onnx": onnx.__version__,
                     "openvino": ov.__version__, "numpy": np.__version__},
    }
    ...
    report = {
        ...
        "prefix": prefix, "io": "uint8",
        "encoder_fp32": {**encoder_info, "op_types": op_inventory(
            onnx.load(str(fp32_path)).graph.node)},
        "decoder": {**decoder_info, "op_types": op_inventory(
            onnx.load(str(decoder_path)).graph.node)},
        "encoder_int8": {"quantization": quantization,
                         "op_types": op_inventory(onnx.load(str(qdq_path)).graph.node),
                         "openvino_ir": {**ir_info, "op_types": op_inventory(
                             core.read_model(ir_info["xml"]).get_ops())}},
        "calibration": {"split": calibration_split, "image_indices": calibration_indices},
