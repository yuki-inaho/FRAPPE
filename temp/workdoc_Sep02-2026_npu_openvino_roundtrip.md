...
- [x] 🖐 **操作**: `PATH="$HOME/.pixi/bin:$PATH" pixi install` を（マージ後の `pixi.lock` で）実行する。
- [x] 🔎 **確認**: 終了コード 0。`pixi run python -c "import torch, onnx, onnxscript, onnxsim, openvino; print(torch.__version__, openvino.__version__)"` が動く。
- [x] 🧪 **テスト**: 上記 import が例外なく通ること。初期状態では `ModuleNotFoundError` で失敗するはずで、それが成功へ変わることを確認する。
- [x] 🛠 **エラー時対処**: ダウンロードが遅い/落ちる場合は再実行する（rattler と uv のキャッシュが効くので進捗は失われない）。CUDA 版 torch が不要という判断に至った場合は、**既定環境を書き換えず** `[feature]` として CPU 版環境を追加する形にし、その理由を本書に記録する。

### 手順 4: 既存テストで基準線を取る
- [x] 🖐 **操作**: `pixi run test` を実行する。
- [x] 🔎 **確認**: 全件通過。件数を記録する（`2426b37` 時点で 66 件 + `adapt_to_decoder` の 5 パラメータ = 71 件見込み）。
- [x] 🧪 **テスト**: `tests/test_prefix_model.py::test_adapt_to_decoder_matches_the_einops_formulation` が 5 通りすべて通ること（新契約の前提そのもの）。
- [x] 🛠 **エラー時対処**: 失敗したテストがこの機械固有（GPU 前提など）かを切り分ける。環境由来なら理由を記録し、コード由来なら先に直す。

### 手順 5: 新契約 ONNX を CR-50 で再エクスポートする
- [x] 🖐 **操作**: `pixi run export-onnx --checkpoint "$FRAPPE_BUNDLE_ROOT/model_cr50/final16.pth.tar" --output-stem .private/onnx/frappe_cr50_16ch --io uint8 --dataset-root "$FRAPPE_LOCAL_DATA_ROOT/imagefolder" --split validation --report .private/onnx/cr50_report.json`
- [x] 🔎 **確認**: 標準出力の入出力署名が `image UINT8 [1, 3, '32*units_h', '32*units_w']` と scale 群ごとの `plane_p*` になっている。自己検証が「平面 0 不一致 / JPEG-LS byte-identical / 4 解像度すべて ok」で終わる。
- [x] 🧪 **テスト**: ツール自身の検証が gate（`mismatched_symbols != 0` または payload 不一致なら `SystemExit`）。ここを通ることが合格条件。
- [x] 🛠 **エラー時対処**: dynamo exporter が落ちる場合は `onnxscript` の導入を確認する。`--no-simplify` で onnxsim を切って切り分ける。解像度再実行で落ちる場合は symbolic dim の関係式が壊れているので、`describe()` の出力を本書に貼って原因を特定する。

### 手順 6: 新契約 ONNX を CR-40 でも再エクスポートする
- [x] 🖐 **操作**: 手順 5 と同じコマンドを `model_cr40/final17.pth.tar` / `frappe_cr40_17ch` で実行する。
- [x] 🔎 **確認**: 手順 5 と同じ合格条件。scale 群の構成が 17ch 側（`ps=[32,16×5,8×3,4×6,2×2]`）になっている。
- [x] 🧪 **テスト**: 同上。
- [x] 🛠 **エラー時対処**: 同上。片方だけ失敗する場合はチェックポイントの `config.ps` を突き合わせる。

### フェーズ 1: OpenVINO デバイス制約の確定

### 手順 7: 新グラフを 3 デバイスでコンパイルできるか確かめる
- [x] 🖐 **操作**: 再エクスポートした encoder / decoder を `openvino.Core().read_model` で読み、800×608 相当に `reshape` して `compile_model` を CPU / GPU / NPU で試すプローブを書いて実行する。
- [x] 🔎 **確認**: 各デバイスについて「成功 / 失敗（例外型とメッセージ）」が表になって出る。失敗は握り潰さず記録する。
- [x] 🧪 **テスト**: 成功したデバイスで 1 回推論し、出力形状が期待どおり（`plane_p32` が `(1, n_s*19, 25)` 等）であること。
- [x] 🛠 **エラー時対処**: NPU が動的形状で拒否する場合は reshape 後に再試行する。特定オペレータで落ちる場合はメッセージからオペレータ名を特定し、手順 8 の対象に加える。

### 手順 8: 落ちたオペレータの回避策を決める
- [x] 🖐 **操作**: 手順 7 で NPU / GPU が拒否したオペレータがあれば、それだけを含む最小グラフを作って再現させ、回避策（グラフ分割・`--io float` 形・当該部分の CPU 実行）を比較する。
- [x] 🔎 **確認**: 採用する回避策と、採らなかった選択肢の理由が本書に書かれている。
- [x] 🧪 **テスト**: 回避策適用後のグラフが対象デバイスで compile でき、出力が回避前と一致すること。
- [x] 🛠 **エラー時対処**: 回避策が無い場合は「そのデバイスではこのグラフは動かない」と明記する。動くふりをしない。

### 手順 9: デバイスプロパティの既定値を決める
- [x] 🖐 **操作**: `core.get_property(device, "SUPPORTED_PROPERTIES")` で実在を確認した上で、`CACHE_DIR` / `INFERENCE_PRECISION_HINT` / `PERFORMANCE_HINT` / `NPU_TURBO` の既定値を決める。
- [x] 🔎 **確認**: 存在しないプロパティを設定していない。既定値の選択理由が本書にある。
- [x] 🧪 **テスト**: 既定値ありとなしで 1 回ずつ推論し、レイテンシ差を記録する。
- [x] 🛠 **エラー時対処**: プロパティ設定で例外が出る場合はそのデバイスの `SUPPORTED_PROPERTIES` を出力して、実在するものだけに絞る。

### フェーズ 2: OpenVINO ランタイムの実装

### 手順 10: ランタイムのテストを先に書く（Red）
- [x] 🖐 **操作**: `tests/test_openvino_runtime.py` に、(a) CPU デバイスで encoder の平面が PyTorch 参照とバイト一致する、(b) 存在しないデバイスを明示指定すると例外になる、(c) `auto` は使えるデバイスを選び理由を返す、の 3 テストを書く。
- [x] 🔎 **確認**: 3 件とも `ModuleNotFoundError` または `AttributeError` で失敗する（実装が無いため）。
- [x] 🧪 **テスト**: `pixi run pytest tests/test_openvino_runtime.py -q` が 3 failed になること。
- [x] 🛠 **エラー時対処**: 失敗理由が意図と違う（例: fixture のパス誤り）場合はテスト側を直してから進む。

### 手順 11: ランタイムを実装する（Green）
- [x] 🖐 **操作**: `src/compressors/frappe/openvino_runtime.py` を実装する。ONNX パス・デバイス・解像度を受け取り、reshape → compile → 推論。encoder と decoder を別クラスにし、コンパイル済みモデルをキャッシュする。
- [x] 🔎 **確認**: 手順 10 の 3 件が通る。
- [x] 🧪 **テスト**: `pixi run pytest tests/test_openvino_runtime.py -q` が 3 passed へ変わること。
- [x] 🛠 **エラー時対処**: バイト不一致が出た場合、まず入力の正規化（uint8 のまま渡しているか）を疑う。新契約では正規化はグラフの中なので、外で `/127.5-1` をかけていたら誤り。

### 手順 12: NPU / iGPU でのビット一致を確認する
- [x] 🖐 **操作**: 同じテストを NPU と iGPU に対して実行する（デバイス列挙で存在する場合のみ）。
- [x] 🔎 **確認**: 3 デバイスすべてで符号平面が PyTorch 参照とバイト一致する。一致しないデバイスがあれば、不一致シンボル数と最大差を記録する。
- [x] 🧪 **テスト**: `plane_mismatched_symbols == 0` を assert する。fp16 実行で一致しない場合は、それが `INFERENCE_PRECISION_HINT` に起因することを実験で示す。
- [x] 🛠 **エラー時対処**: 不一致が出たら `INFERENCE_PRECISION_HINT=f32` で再試行する。それでも不一致なら、そのデバイスは符号器に使えないと明記し、デコーダ専用にする。

### フェーズ 3: ラウンドトリップと計測

### 手順 13: ラウンドトリップのテストを先に書く（Red）
- [x] 🖐 **操作**: `tests/test_roundtrip_openvino.py` に、合成画像 1 枚で encode → JPEG-LS → decode が通り、JPEG-LS 往復後の平面が符号器出力と一致することを検証するテストを書く。
- [x] 🔎 **確認**: 実装が無いため失敗する。
- [x] 🧪 **テスト**: `pixi run pytest tests/test_roundtrip_openvino.py -q` が failed。
- [x] 🛠 **エラー時対処**: 合成画像のサイズは 32 の倍数かつ `units >= 2`（すなわち最小 64×64）にする。これを外すと reflect padding の制約に当たる。

### 手順 14: ラウンドトリップ CLI を実装する（Green）
- [x] 🖐 **操作**: `tools/roundtrip_openvino.py` を実装する。`--encoder-device` / `--decoder-device` / `--checkpoint-onnx-stem` / `--image-index` / `--output` / `--report`。JPEG-LS は `entropy_coding.encode_latents` / `decode_latents` をそのまま使う。
- [x] 🔎 **確認**: 手順 13 のテストが通る。私有データ 1 枚で実行して再構成 PNG が出る。
- [x] 🧪 **テスト**: JSON に `plane_mismatched_symbols == 0`、`psnr_db`、`bpp_payload_only`、`bpp_with_length_prefix`、段別レイテンシが載ること。
- [x] 🛠 **エラー時対処**: `decode_latents` が返す平面の形が合わない場合、新契約では平面が `(C*H, W)` である点と、デコーダグラフが `unflatten` を内蔵している点を突き合わせる。

### 手順 15: 6 通りのデバイス組み合わせを測る
- [x] 🖐 **操作**: `tools/benchmark_openvino_devices.py` を実装・実行する。encoder × decoder × {CPU, GPU, NPU}、初回コンパイル込みと定常状態を分離、JPEG-LS の CPU 時間も別途計上する。
- [x] 🔎 **確認**: 表が JSON と標準出力に出る。どの組み合わせが最速かが読み取れる。
- [x] 🧪 **テスト**: 各測定が最低 10 回の中央値であること。ウォームアップを計測から除外していること。
- [x] 🛠 **エラー時対処**: 測定がばらつく場合は電源モードと他プロセスの負荷を記録し、繰り返し数を増やす。

### 手順 16: 62 枚全体で評価する
- [x] 🖐 **操作**: 最速構成と「NPU で符号化する構成」の両方で、62 枚の PSNR / bpp / CR を測る。
- [x] 🔎 **確認**: 両構成で符号がビット一致し、したがって bpp と PSNR が完全に同一であること（デバイスは速度だけを変え、結果を変えない）。
- [x] 🧪 **テスト**: 2 つの JSON の `psnr_db` と `bpp_payload_only` が厳密一致すること。
- [x] 🛠 **エラー時対処**: 一致しない場合は手順 12 に戻る。デバイスによって結果が変わるのは、この作業では欠陥として扱う。

### フェーズ 4: JPEG-LS の OpenVINO オフロード（副目標）

### 手順 17: CPU 参照 MED/Qs をオラクルとして用意する
- [x] 🖐 **操作**: 参考実装の CPU 参照計算（境界付き行バッファ、MED、勾配量子化、`Qs`）を、本リポジトリのテストから呼べる形にする。MIT ライセンスの帰属を残す。
- [x] 🔎 **確認**: FRAPPE の実際の潜在平面（例: CR-50 の `plane_p2` が 304×400）に対して MED/Qs が計算できる。
- [x] 🧪 **テスト**: 参考実装の既存テストのうち、NPU を要さないものが通ること。
- [x] 🛠 **エラー時対処**: 参考実装が現行 Python で動かない場合は、必要な関数だけを移植し、移植元をコメントで明記する。
- [x] 🛠 **判断ポイント**: この時点で「CharLS より速くなる見込みが無い」と判明した場合は、実装ではなく**測定結果と判断を記録して終える**。副目標を目的化しない。

### 手順 18: OpenVINO で MED/Qs を計算する
- [x] 🖐 **操作**: A/B/C/D 抽出 → 分岐なし MED → 勾配量子化 → `Qs` を OpenVINO グラフとして構築し、iGPU / NPU で実行する。
- [x] 🔎 **確認**: 出力が CPU 参照と**完全一致**（整数値なので厳密一致を要求する）。
- [x] 🧪 **テスト**: 全タイル比較で不一致 0。不一致が 1 つでもあれば符号化を中止する（参考実装と同じ規約）。
- [x] 🛠 **エラー時対処**: FP16 で表現範囲を外れる場合は、参考実装 `CONCEPT_JA.md` §7 の範囲議論を確認し、必要なら `INFERENCE_PRECISION_HINT=f32` を使う。

### 手順 19: ffmpeg で外部相互運用を確認する
- [x] 🖐 **操作**: 生成した `.jls` を `ffmpeg -i out.jls -f rawvideo -` で復号し、元の平面と一致することを確認する。
- [x] 🔎 **確認**: 全画素一致。
- [x] 🧪 **テスト**: 自前復号器と ffmpeg の双方で一致すること（自作同士が同じ誤りを持つ可能性を排除する）。
- [x] 🛠 **エラー時対処**: ffmpeg が読めない場合はマーカー構造を `npu-jpegls inspect` 相当で確認する。

### フェーズ 5: 検証・記録

### 手順 20: 品質ゲートを通す
- [x] 🖐 **操作**: `pixi run test`、`pixi run ruff check .`、`pixi run ruff format --check .` を実行する。
- [x] 🔎 **確認**: すべて成功。追加したテストの件数を記録する。
- [x] 🧪 **テスト**: 追加テストを含む全件が通ること。
- [x] 🛠 **エラー時対処**: lint の失敗は自動修正可能なものと設計上の指摘を切り分けてから対処する。

### 手順 21: 私有データ非混入を検査する
- [x] 🖐 **操作**: `git status --porcelain` と、`git diff --cached` および追加した全ファイルに対する私有パス文字列（`2026_TW_TVA` など）の検索を行う。
- [x] 🔎 **確認**: 検索結果 0 件。`.private/` 配下が一切 staging されていない。
- [x] 🧪 **テスト**: `git grep -I -n '2026_TW_TVA' $(git ls-files)` が何も返さないこと。
- [x] 🛠 **エラー時対処**: 1 件でも出たら該当箇所を環境変数参照へ書き換え、コミット前に必ず再検査する。既にコミットしていたら履歴の扱いをユーザーに確認する。
...
- [x] **DoD-1 (目標1 / TR-1,2,3)**: CR-40 と CR-50 の両方で、私有評価画像 1 枚に対し
      `encode → JPEG-LS → decode → 再構成 PNG` が 1 コマンドで完走し、再構成 PNG と
      レポート JSON が生成されている。encoder を NPU に置いた構成でも完走する。
- [x] **DoD-2 (TR-4)**: OpenVINO の CPU / iGPU / NPU が出す符号平面が、PyTorch 参照と
      **バイト一致**する。一致しないデバイスがある場合は、不一致数・最大差・原因・
      そのデバイスを符号器に使わない判断が本書に記録されている。
- [x] **DoD-3 (TR-8)**: encoder × decoder × {CPU, GPU, NPU} のレイテンシが、初回コンパイル込みと
      定常状態に分けて測定され、JSON に残っている。どの構成を採るかの判断が数字に紐づいている。
- [x] **DoD-4 (TR-7)**: 62 枚での PSNR / bpp / CR が、長さ接頭辞なし・込みの**両規約**で
      記録され、既知の CR40 (29.78 dB / CR 40.47) / CR50 (28.63 dB / CR 50.53) と
      同一規約で並べられる形になっている。
- [x] **DoD-5 (TR-5)**: `--device NPU` を明示して NPU が使えない状況を作ったとき、CPU に
      落ちずに例外で止まることがテストで示されている。
- [x] **DoD-6 (TR-6)**: `git status --porcelain` に私有物が現れず、追跡対象の全ファイルに
      私有パス文字列が含まれないことが検査で確認されている。
- [x] **DoD-7 (TR-9)**: 配備に使う ONNX が `2426b37` の新契約（uint8 I/O、symbolic dims、
      平面出力）で再エクスポートされ、その自己検証が通っている。旧 ONNX は配備に使っていない。
- [x] **DoD-8 (TR-10 / 副)**: JPEG-LS の OpenVINO オフロードについて、実装できたなら
      CPU 参照との完全一致と ffmpeg 相互運用が示されている。実装しない判断をしたなら、
      その根拠（CharLS との速度見積もり）が数字で記録されている。
- [x] **DoD-9**: `pixi run test` と lint が通り、追加したテストが回帰を守る形になっている。
- [x] **DoD-10**: §1.3 の全 Trace ID に証跡が対応している。未達の項目は未達と明記されている。
...
| 2026-09-02 | 09:30 JST | Claude | ONNX 再エクスポート (手順 5, 6) | CR-50 16ch / CR-40 17ch を `--io uint8` で再エクスポート。自己検証「平面 0 不一致 / JPEG-LS byte-identical / 4 解像度 ok」を通過。`.private/onnx/{cr40,cr50}_report.json` に記録 |
| 2026-09-02 | 09:50 JST | Claude | openvino_runtime + ツール2本 + bitstream 実装 (手順 7〜14) | TDD: `test_openvino_runtime.py` 12 failed → 12 passed。メタデバイス拒否・明示失敗・インデックス束縛・`plane_shapes_for` 静的化。旧グラフ実機配線検証で CPU/GPU.0/NPU すべて `EXECUTION_DEVICES` 確認。コミット `1cb23f5` |
| 2026-09-02 | 09:52 JST | Claude | **旧グラフ実測** (手順 15 の旧契約版) | encoder CPU 1.70 / GPU 16.07 / NPU 17.70 ms、decoder CPU 213.11 / GPU 45.35 / NPU 178.21 ms。NPU decoder は初回 ~355 ms→定常 ~176 ms の ramp を確認（ウォームアップ不足のベンチは 2 倍過小評価する） |
| 2026-09-02 | 10:00 JST | Claude | main のリファクタ 9 コミットをマージ (4eb5791) | harness 新設に合わせ自作 `bitstream.py` を削除し `harness.bitstream` に `decode_planes` を追加。ツールを harness 規約へ移行、コミット `dbff089` |
| 2026-09-02 | 10:30 JST | Claude | JPEG-LS front half を OpenVINO で実装 (手順 17, 18 / SG-6) | `jpegls_openvino.py`。**重要発見**: (1) この NPU は `IE.Minimum` を legalize できず、しかもエラーを出しつつ壊れた blob を返す。(2) 比較経路 `Convert(Greater)` 9345/9500、`Select(Greater)` 1366/9500 が**静かに誤る**。算術形 `Sign(ReLU(m−t))` / `Clamp` / min-max MED は 3 デバイス全形状で厳密。(3) 修正版を全10平面形状+退化5種+内容5種で再検証 → WRONG=0 (NPU 1×1 のみクリーンなコンパイル拒否) |
| 2026-09-02 | 10:35 JST | Claude | テスト追加とコミット (手順 20 の一部) | `tests/test_jpegls_openvino.py` 27 tests（独立 CPU 定式化との交叉検証・known patch 手計算アンカー・全デバイス exact）。`pixi run test` 全緑 (186 passed + 2 skipped)。コミット `1a0c337` (.gitignore), `5a24353` (module+tests) |
| 2026-09-02 | 10:40 JST | Claude | JPEG-LS オフロードの費用対効果 (手順 15 補完) | `tools/benchmark_jpegls_openvino.py` を作成し実測: front half だけで CPU 1.80 / iGPU 3.38 / NPU 146.01 ms、CharLS 全体 1.86 ms。fp16 入力版も exact かつ同速度 (146.8 ms) → NPU の f32 strawman を排除。コミット `91e4ec6`。**副目標はレイテンシ目的では測定付きの否定的結論として閉じ (DoD-8)** |
| 2026-09-02 | 10:44 JST | Claude | ラウンドトリップ実走 (手順 13, 14) | CR-50: CPU(bit-exact)+NPU decode 128.17 ms, PSNR 27.527 dB / CR 48.50、PyTorch 参照検証 `plane_mismatched=0`。CR-40: 130.34 ms, 28.574 dB / CR 39.00、同 0。**encoder NPU + decoder GPU 構成も完走** (70.86 ms, DoD-1)。マージ後初回実行で `Table` API 不一致のバグを発見・修正 |
| 2026-09-02 | 10:46 JST | Claude | 62 枚評価 (手順 16, SG-5) | CR-50 × CPU encode + NPU decode: PSNR 27.729 dB / 0.47747 bpp (payload-only) / 0.47780 bpp (接頭辞込み) / CR 50.265。既知の CR-50 点 (28.63 dB / CR 50.53) と同一規約で比較可能。`.private/out/cr50_roundtrip_62img_npu.json` |
| 2026-09-02 | 10:48 JST | Claude | 6 通りデバイスベンチ (手順 15, DoD-3) | steady median: encode CPU 1.72 / GPU 15.38 / NPU 18.60 ms, decode CPU 121.56 / GPU 48.05 / NPU 124.74 ms。最速 warm 構成 = **encode CPU + JPEG-LS CPU + decode GPU 51.95 ms**。`.private/out/benchmark_devices.json` |
| 2026-09-02 | 10:52 JST | どちらも | **NPU 不採用の意思決定** | NPU は decode で GPU の 2.6 倍遅く (124.7 vs 48.1 ms)、encode でも CPU の 11 倍遅い。ウォームアップ ~15 回・形状固定コンパイルの制約もあり、**本ワークロードへの NPU 採用は取りやめ** (ユーザー判断、上記実測が根拠)。NPU decoder 書き換え案 (176→44 ms 見込み) も着手しない |
| 2026-09-02 | 10:55 JST | どちらも | JPEG XL lossless e1 vs CharLS の実測 | `tools/benchmark_entropy_codecs.py` (コミット `3ce38a6`)。プロセス内計測 (pillow_jxl バンドル libjxl 0.12.0; apt の cjxl 0.7.0 は古く CLI 起動コストも計測を歪めるため未使用)。62 枚の実平面で: CharLS encode 1.85 ms / 29,030 B (0.690 b/sample)、**JXL e1 encode 5.85 ms / 41,060 B** (3.2× 遅く 41% 大きい)、JXL e3 は 7.2× 遅くサイズ同等。decode も JXL が 3.6× 遅い。→ **CharLS が全軸で勝ち、座を維持** |
| 2026-09-02 | 10:56 JST | どちらも | 「encoder 部 ~1ms」への回答 | 段別実測: 分析路 (encoder graph, CPU bit-exact) **1.60 ms** + CharLS 1.85 ms ≈ 3.45 ms。エントロピー段を JXL に入れ替えても 1 ms には届かない (~7.5 ms)。**~1 ms はエントロピー段の交換では到達不可** (分析路単体で既に 1.6 ms)。分析路の高速化 (量子化後の f32 conv の削減等) が次のレバー |
