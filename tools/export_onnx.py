...
class EncoderGraph(torch.nn.Module):
    ...
    def forward(self, image: torch.Tensor):
        ...
        for analysis, compander in zip(self.analysis, self.companders):
            codes = torch.clamp(
                torch.round(compander.companded(analysis(x))), -CODE_OFFSET, CODE_OFFSET
            )
            ...
            planes.append(
                (plane + CODE_OFFSET).to(torch.uint8) if self.uint8_io else plane.to(torch.int8)
            )
        return tuple(planes)
...
class DecoderGraph(torch.nn.Module):
    ...
    def forward(self, *planes: torch.Tensor):
        ...
        if self.uint8_io:
            return torch.clamp(
                torch.round((reconstruction.clamp(-1.0, 1.0) / 2 + 0.5) * 255.0), 0.0, 255.0
            ).to(torch.uint8)
...
def dynamic_shapes(
    ...
    ``batch_max`` above one makes the batch a ``[1, batch_max]`` dimension
    instead of a fixed one, and passing ``height``/``width`` pins both spatial
    axes -- a deployment that serves one resolution can bake it in and skip
    the reshape step at load time. Pinning and a batch range compose.
    """
    from torch.export import Dim

    if height is not None and width is not None:
        from src.compressors.frappe.openvino_runtime import plane_shapes_for

        batch = Dim("batch", min=1, max=batch_max) if batch_max > 1 else 1
        image = {0: batch, 2: height, 3: width}
        planes = tuple(
            {0: batch, 1: rows, 2: cols} for rows, cols in plane_shapes_for(model.ps, height, width)
        )
        return image, planes
    ...
    planes = tuple(
        {1: (end - start) * (max_ps // ps) * units_h, 2: (max_ps // ps) * units_w}
...
def describe(path: Path) -> dict:
    ...
    return {
        "inputs": signature([v for v in model.graph.input if v.name not in initialisers]),
...
def verify_against_reference(
    model, decoder, folder, encoder_session, decoder_session, plane_names, count, uint8_io
) -> dict:
    ...
    for index in range(min(count, len(folder))):
        ...
        if index == 0:
            reference_blob = len(
                encode_planes(arrange_planes(codes), BitstreamConvention.WITH_LENGTH_PREFIX)
            )
        ...
        for reference, candidate in zip(reference_planes, onnx_planes):
            emitted = (
                candidate[0]
                if uint8_io
                else (candidate[0].astype(np.int32) + int(CODE_OFFSET)).astype(np.uint8)
            )
            worst_plane = max(
                worst_plane,
                int(np.abs(emitted.astype(np.int32) - reference.astype(np.int32)).max()),
            )
            mismatched += int((emitted != reference).sum())
            symbols += reference.size
            if encode_plane(torch.from_numpy(emitted)) != encode_plane(torch.from_numpy(reference)):
                payload_mismatch += 1
        (onnx_recon,) = decoder_session.run(None, dict(zip(plane_names, onnx_planes)))
        with torch.no_grad():
            expected = decoder(*[torch.from_numpy(plane) for plane in onnx_planes])
        difference = np.abs(expected.numpy().astype(np.float64) - onnx_recon.astype(np.float64))
    ...
    print(
        f"    encoder planes : max |difference| = {worst_plane}   "
        f"mismatched symbols = {mismatched}/{symbols}"
    )
    payload_note = (
        "byte-identical" if payload_mismatch == 0 else f"{payload_mismatch} streams differ"
    )
    print(
        f"    JPEG-LS payload: {payload_note}   (reference blob for image 0: "
        f"{reference_blob} B, length prefixes included)"
    )
    print(
        f"    decoder output : max |difference| vs PyTorch = {worst_recon:g}"
    ...
    if not uint8_io and worst_recon > 1e-3:
        raise SystemExit("ONNX decoder output diverges from PyTorch")
    return {
        "images": min(count, len(folder)),
        "max_plane_difference": worst_plane,
...
def verify_resolutions(
    model, encoder_session, decoder_session, plane_names, uint8_io, height: int, width: int
) -> list[list[int]]:
    """A graph that only runs at the traced size is the defect this export avoids."""
    max_ps = max(model.ps)
    sizes = [(height, width), (max_ps * 2, max_ps * 3), (max_ps * 10, max_ps * 15), (1088, 1920)]
    print("\n  same graphs re-run at other resolutions:")
    for probe_height, probe_width in sizes:
        probe = np.zeros(
            (1, model.input_channels, probe_height, probe_width),
...
def measure_latency(
    encoder_session, decoder_session, plane_names, image, repeats: int
) -> tuple[float, float]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="1 keeps the batch fixed (the default, and what an image "
        "codec deploys); N above one makes the batch dimension "
        "dynamic over [1, N], for batched deployments",
    )
    parser.add_argument(
        "--pin-size",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="bake --export-height/--export-width into the graph as "
        "constants, skipping the runtime reshape; the resolution "
        "re-verification does not apply to a pinned graph and is "
        "skipped",
    )
    parser.add_argument(
        "--opset",
    ...
    parser.add_argument(
        "--threads", type=int, default=1, help="onnxruntime intra-op threads for the timing report"
    )
    ...
    image_shape, plane_shapes = dynamic_shapes(
        model,
        batch_max=args.export_batch_max,
        height=args.export_height if args.pin_size else None,
        width=args.export_width if args.pin_size else None,
    )
    encoder = EncoderGraph(model, uint8_io).eval()
    decoder = DecoderGraph(model, prefix, uint8_io).eval()
    # A dynamic batch is traced on a batch of two: with a batch-one sample the
    # tracer derives a "batch <= 2" guard from the decoder's broadcast pattern
    # and refuses the declared range. Two sits inside every range and traces
    # the broadcasts honestly.
    sample_batch = min(2, args.export_batch_max)
    sample_image = (
        torch.zeros(
            sample_batch, model.input_channels, args.export_height, args.export_width, dtype=torch.uint8
        )
        if uint8_io
        else torch.zeros(sample_batch, model.input_channels, args.export_height, args.export_width)
    )
    with torch.no_grad():
    ...
    for path, info in ((encoder_path, encoder_info), (decoder_path, decoder_info)):
        note = (
            f"  simplified {info['nodes_before']} -> {info['nodes_after']} nodes, "
        ...
        for direction in ("inputs", "outputs"):
            for entry in signature[direction]:
                print(
                    f"    {direction[:-1]:6s} {entry['name']:14s} "
    ...
    # A pinned graph answers one resolution by construction, so re-running it
    # at others would test nothing and fail loudly instead.
    resolutions = (
        [
            {
                "size": [args.export_width, args.export_height],
                "ok": True,
                "note": "pinned: the graph has no other resolution",
            }
        ]
        if args.pin_size
        else verify_resolutions(
            model,
            encoder_session,
            decoder_session,
            plane_names,
    ...
    encode_ms, decode_ms = measure_latency(
        ...
        decoder_session,
        plane_names,
        folder.pixels(0) if uint8_io else folder.signed(0),
        args.timing_repeats,
    )
    pixels = args.export_height * args.export_width
    print(f"\n  CPU latency at {args.export_width}x{args.export_height}, {args.threads} thread(s):")
