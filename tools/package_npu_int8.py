...
def parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument("--verify-images", type=int, default=16,
                        help="images measured through the shipped artifacts for "
                             "the manifest's RD baseline")
    parser.add_argument("--height", type=int, default=608,
                        help="resolution the OpenVINO IR is frozen to")
    parser.add_argument("--width", type=int, default=800)
    parser.add_argument("--prefix", type=int, default=None,
                        help="operating point; default: the checkpoint's full n_channels")
    parser.add_argument("--target-device", default="NPU", choices=["NPU", "CPU", "ANY"],
                        help="placement policy for the deployed graph, not this machine")
    parser.add_argument("--ptq-preset", default="performance",
                        choices=["performance", "mixed"])
    parser.add_argument("--bias-correction", default="fast",
                        choices=["fast", "accurate", "none"])
    parser.add_argument("--device", default="cuda:0",
                        help="torch device for the frozen decoder in the RD baseline; "
                             "CPU is allowed")
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        manifest = build_package(
            loaded.model, args.checkpoint, args.dataset_root, args.output_dir,
            calibration_split=args.calibration_split,
            calibration_images=args.calibration_images,
            verify_split=args.verify_split, verify_images=args.verify_images,
            height=args.height, width=args.width, prefix=args.prefix,
            target_device=args.target_device, ptq_preset=args.ptq_preset,
            bias_correction=args.bias_correction, decode_device=args.device,
